...
:ID:       uq35rvn02wj0
:END:
#+title: [draft][don't take it seriously] Go Incomplete Guide
#+DATE: <2022-01-09 Sun 18:51>
#+draft: false
...
* Misc
** Notes
- Reading from a nil channel would result in blocking forever.
- Writing to a nil map will cause a panic.
- Ranging over a nil slice will result in 0 iterations.
- First is a type alias. Second is a new type.
  #+begin_example
type MyInt int
type YourInt = int
  #+end_example
- iota resets per const block. When you manually assign (C = 5), it doesn’t stop iota from incrementing in the background.

- byte is an alias for uint8. rune is an alias for int32 (Unicode code point). string is a read-only slice of bytes, UTF-8 encoded. Iterating a string with for range gives runes, not bytes. But indexing a string with [i] gives bytes.
...
for i := 0; i < 3; i++ {
  ...
  This prints 3 3 3. Because i is captured by reference not by value.

- =defer= might not be always useful inside a loop:
  #+DOWNLOADED: screenshot @ 2025-11-06 21:28:58
  [[file:../../media/Misc/2025-11-06_21-28-58_screenshot.png]]

- Go will try to store values in the stack, however, for long-lived variables, they will be stored in the heap. https://youtu.be/US3TGA-Dpqo?si=I6Hz_QN4oKU66T42&t=236

- Closures in Go are lexically scoped. This means that any variables referenced within the closure from the "outer" scope are not a copy but are in fact a reference. A for loop actually reuses the same variable multiple times, so you're introducing a race condition between the read/write of the s variable. In general, it is a best practice to pass in any arguments you want so that you don't have references.
...
- Closures capture variables, not values:
  ...
  #+RESULTS:
  However; proposal #60078 changed loop variables to be per-iteration instead of per-loop.

- https://youtu.be/K1hwpNnCJgY?si=bpv9n89CFbfF6m_o You need to keep locking/unlocking in the same order.

- Do not forget the decision tree for using primitives vs. channels.
  #+DOWNLOADED: screenshot @ 2025-11-07 20:38:51
  [[file:../../media/Misc/2025-11-07_20-38-51_screenshot.png]]

- cond. Observe the example:
  #+begin_src go
package main
...
#+end_src
** Custom Sort
To sort a slice in reverse order, you can use the ~Reverse()~ function along with ~StringSlice~,
~IntSlice~, and ~Float64Slice~ types. These types implement the Interface for comparision and
swap defined in the sort package.

The ~Reverse()~ function returns the reverse order of data. Let’s look at an example of
...
For these usecases, you can use the ~Slice()~ and ~SliceStable()~ functions provided by the sort
...
** [[id:6ed1b62f-0992-46dc-aefd-40c471ae0b66][YouTube Channels]]
:PROPERTIES:
:CUSTOM_ID: yt
:END:
- [[https://www.youtube.com/@mattkdvb5154][Matt KØDVB]]
- [[https://www.youtube.com/@GolangCafe][Golang Cafe]]
- [[https://www.youtube.com/@JustForFunc][justforfunc: Programming in Go]]
- [[https://www.youtube.com/@NicJackson][Nic Jackson]]
- [[https://www.youtube.com/@anthonygg_][Anthony GG ]]
- [[https://www.youtube.com/@MarioCarrion][Mario Carrion]]
- [[https://www.youtube.com/@rscgolang][Russ Cox]]
- [[https://www.youtube.com/@GopherAcademy][Gopher Academy]]
- [[https://www.youtube.com/@GopherConUK][GopherCon UK]]
- [[https://www.youtube.com/@gojax][Go Jax]]
- [[https://www.youtube.com/@DonaldFeury][Donald Feury]]
- [[https://www.youtube.com/@NerdCademyDev][NerdCademy]]
- [[https://www.youtube.com/@boldlygo][Boldly Go ]]
- [[https://www.youtube.com/@joncalhoun][Jon Calhoun]]
- [[https://www.youtube.com/@Tutorialedge][TutorialEdge]]
- [[https://www.youtube.com/@TECHSCHOOLGURU][TECH SCHOOL]]
- [[https://www.youtube.com/@GolangDojo][Golang Dojo]] (Worst)
- [[https://www.youtube.com/@mathisve][Mathis Van Eetvelde]]
* Old
These are notes I wrote when I was still learning Go.
** Introduction
...
See [[id:03ogh2p02wj0][Concurrency Patterns in Go]]
**** Goroutines
...
#+end_src
**** Note about error handling in Go                                 :noexport:

:DATE:
[2023-08-04 Fri 07:19]
:DATE:
There is nothing wrong about error handling in Go. It is one of them most
idiomatic error handling methods ever, unlike many people I think it is one of
the  best features of this language, if you disagree you are brainwashed by
exception handling (for real, if you think it is not good because exceptions are
better you are brainwashed, otherwise we can have a discussion).

Anyway, you can notice that I'm writing this after about 2 years from the date
of publishing this post. This is to add some goodies update that the Go team
made about erros
